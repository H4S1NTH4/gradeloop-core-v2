...
function validateCreate(v: CreateFacultyRequest): AcademicFormErrors {
  ...
  if (!v.leaders || v.leaders.length === 0) {
    ...
    v.leaders.forEach((l, i) => {
      ...
      else if (l.role.trim().length < 3) e[`leader_role_${i}`] = 'Minimum 3 characters';
    });
  }
  return e;
}
...
function LeadersEditor({
  ...
  function setLeader(
    ...
    const next = leaders.map((l, i) => (i === index ? { ...l, [field]: value } : l));
    onChange(next);
              ...
              <div className="grid grid-cols-2 gap-2 col-span-1">
                ...
                <div className="space-y-1">
                  ...
                  {employeesLoading ? (
                    ...
                    <div className="relative">
                      <select
                        className="flex h-9 w-full appearance-none rounded-md border border-zinc-200 bg-black px-3 py-1 pr-8 text-sm text-zinc-900 dark:text-zinc-50 shadow-xs transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-zinc-950 dark:border-zinc-800 dark:focus-visible:ring-zinc-300"
                  ...
                  {errors[`leader_uid_${i}`] && (
                    <p className="text-xs text-red-600">{errors[`leader_uid_${i}`]}</p>
                  )}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {errors[`leader_role_${i}`] && (
                    <p className="text-xs text-red-600">{errors[`leader_role_${i}`]}</p>
                  )}
                </div>
              </div>
...
export function CreateFacultyDialog({
  ...
  const [values, setValues] = React.useState<CreateFacultyRequest>(EMPTY_CREATE);
  ...
  function setField(field: keyof Omit<CreateFacultyRequest, 'leaders'>, value: string) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new top-level faculty. At least one employee leader is required.
          </DialogDescription>
        </DialogHeader>
...
export function EditFacultyDialog({
  ...
  function setField(field: keyof Omit<UpdateFacultyRequest, 'leaders' | 'is_active'>, value: string) {
    setValues((prev) => ({ ...prev, [field]: value }));
    if (errors[field]) setErrors((prev) => ({ ...prev, [field]: undefined }));
  }
